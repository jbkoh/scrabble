...
def crf_result_acc():
    ...
    source_target_list = [('ebu3b', 'ap_m'), ('ebu3b', 'ghc')]
...
#fig, ax = plt.subplots(1, 1)
    ...
    filename_template = 'result/crf_{target_building}_{source_buildings}_{source_num}_{exp_id}.json'
    n_s_list = [400, 200, 0]
    #n_s_list = [1000, 200, 0]
    postfixes = ['0', '1']
    ...
    for i, (ax, (source, target)) in enumerate(zip(axes, source_target_list)):
        ...
        for n_s in n_s_list:
            ...
            mf1_cands = []
            if n_s == 0:
                source_buildings = [target]
            else:
                source_buildings = [source, target]
            for postfix in postfixes:
                ...
                with open(filename, 'r') as fp:
                    data = json.load(fp)
                x_cand = [datum['learning_srcids'] for datum in data]
                f1_cand = []
                mf1_cand = []
                for datum in data:
                    f1_cand.append(datum['metrics']['microf1'] * 100)
                    mf1_cand.append(datum['metrics']['macrof1'] * 100)
                x_cands.append(x_cand)
                f1_cands.append(f1_cand)
                mf1_cands.append(mf1_cand)
            f1s = average_data(xs, x_cands, f1_cands)
            mf1s = average_data(xs, x_cands, mf1_cands)
            ...
            ytick = range(75,101,5)
            #ytick = range(0,101,20)
            ytick_labels = [str(n) for n in ytick]
            xlim = (-5, xtick[-1]+5)
            ylim = (ytick[0]-2, ytick[-1])
            if i == 0:
                legends = [
                    #'#$\mathrm{B}_\mathrm{S}$:{0}'.format(n_s),
                    '#$\mathrm{{B_S}}$:{0}'.format(n_s),
                    #'#$\mathrm{{B_S}}$:{0}'.format(n_s),
                ]
            else:
                legends = None
            ...
            text = '{0} $\mathrm{{\\Rightarrow}}$ {1}'.format(\
                    anon_building_dict[source],
                    anon_building_dict[target])
            ax.text(0.8, 0.2, text, transform=ax.transAxes, ha='right',
...
def entity_iter_result_simpler():
    source_target_list = [('ap_m', 'ebu3b'),
                          #('ebu3b', 'ap_m'),
                          #('ghc', 'ebu3b')
                          ('ap_m', 'ghc')
                          ]
...
#    axes = [ax]
    ...
    for i, (ax, (source, target)) in enumerate(zip(axes, source_target_list)):
        ...
        filename_template = 'result/ir2tagsets_{buildings}_{aug}_{exp_id}_MLP_brick.json'
        prefixes = [('_'.join([target]*2), 1, 'noaug_nots'),
                    #('_'.join([target]*2), 1, 'aug_nots'),
                    #(''.join([target, source, target]), 'source_nosa'),
                    ('_'.join([target, source, target]), 2, 'aug_nots'),
                    #('_'.join([target, source, target]), 2, 'aug_ts')
                    ]
        #linestyles = [':', '--', '-.', '-']
        linestyles = [':', '-']
        for linestyle, (buildingfix, building_num, optfix) in zip(linestyles, prefixes):
            if '_ts' in optfix and 'ebu3b' not in buildingfix:
                continue
            sa_flag = 'X' if 'noaug' in optfix else 'O'
            src_flag = '0' if building_num == 1 else '200'
            source_num = int(src_flag)
            x_t = range(10,201,10)
            acc_cands = []
            mf1_cands = []
            x_cands = []
            for exp_num in [10, 11, 21]:
                filename = filename_template.format(
                    buildings=buildingfix,
                    aug=optfix,
                    exp_id=exp_num,
                )
                if not os.path.exists(filename):
                ...
                with open(filename, 'r') as fp:
                    #data = json.load(fp)[1:]
                    data = json.load(fp)
                x = [datum['learning_srcids'] for datum in data]
                acc = [datum['metrics']['accuracy'] * 100 for datum in data]
                mf1 = [datum['metrics']['macrof1-all'] * 100 for datum in data]
    ...
    for ax, (source, target) in zip(axes, source_target_list):
        ...
        ax.text(0.4, 0.2, '{0} $\Rightarrow$ {1}'.format(
            anon_building_dict[source], anon_building_dict[target]),
            fontsize=11,
    ...
    legend_order = list(range(0, len(handles)))
    ...
    fig.set_size_inches(4, 1.5)
    save_fig(fig, 'figs/entity_iter_simpler.pdf')
    subprocess.call('./send_figures')
...
if __name__ == '__main__':
    ...
    parser.add_argument(choices= ['crf', 'crf_entity', 'entity_iter',
                                  'etc', 'entity_ts', 'cls', 'word_sim', 'finalizing',
                                  'crf_acc', 'entity_iter_simpler'],
                        dest = 'exp_type')
    ...
    if args.exp_type == 'crf_acc':
        ...
        crf_entity_result()
    elif args.exp_type == 'entity_iter_simpler':
        entity_iter_result_simpler()
