...
def translate_tagset_vector(tagset_vec):
...
def test_brick_tagset(sentence, token_labels, classifier, vectorizer):
    phrases = _bilou_phraser(sentence, token_labels)
...
def batch_test_brick_tagset(sentence_dict, \
    ...
    for srcid, sentence in sentence_dict.items():
        phrase_dict[srcid] = _bilou_phraser(sentence, token_label_dict[srcid])
    ...
    pred_mat = classifier.predict(feature_mat)
    prob_mat =classifier.predict_proba(feature_mat)
    for i, (srcid, pred) in enumerate(zip(srcid_list, pred_mat)):
        pred_tagsets_dict[srcid] = translate_tagset_vector(pred)
...
def learn_brick_tagsets(sentence_dict,
    ...
    classifier.fit(feature_mat, truth_mat)
